...
import { useCreateGroup, useJoinGroup, useLeaveGroup } from "@/lib/data";
  ...
  return (
    <Screen edges={{ top: true, bottom: true }} style={styles.screen}>
      <View style={styles.hero}>
        ...
        <Text variant="title" center style={{ marginTop: spacing.md, lineHeight: 44 }}>{name}</Text>
...
function GroupPickerModal({
  ...
  function confirmLeave(groupId: string, groupName: string) {
    Alert.alert(
      "Sair do bolão",
      `Tem certeza que deseja sair de "${groupName}"? Seus palpites serão mantidos.`,
      [
        { text: "Cancelar", style: "cancel" },
        {
          text: "Sair",
            ...
            {isForm ? (
              ...
              <>
                  ...
                  renderItem={({ item }) => {
                    ...
                    return (
                      <Pressable onPress={() => onSelect(item.id)}
                        ...
                        {item.role !== "owner" && (
                          <Pressable
                            onPress={() => confirmLeave(item.id, item.name)}
                            hitSlop={8}
                            style={styles.leaveBtn}
                          >
                            <Ionicons name="exit-outline" size={20} color={palette.textFaint} />
                          </Pressable>
                        )}
                      </Pressable>
                    );
                ...
                <View style={styles.formActions}>
                  <Button title="Entrar" variant="secondary" icon="enter-outline"
...
const styles = StyleSheet.create({
  ...
  hero: { alignItems: "center", paddingTop: spacing.xxl, marginBottom: spacing.xl },
